...
<div class="tab-content" style="max-height: 85%;" v-if="userSearch == '' ">
    ...
    <div class="tab-pane " id="tabItem1">
          <div class="nk-msg-list" >
            <template v-for="quote in orderedUsers" v-if="quote.quote.status == 'won'" >
            <div class="nk-msg-item"  v-bind:class="{current: quote.id == quoteChat.id }" v-on:click="openChat($event,quote)">
                <p style="display:none">{{wonJob + 1}}</p>
               <div class="nk-msg-media user-avatar" v-if="quote.chatsp.user_img">
                      <img :src="hostname+'/frontend-assets/images/users/'+quote.chatsp.user_img" alt="">
                    </div>
                    <div class="user-avatar bg-purple" v-else>
    ...
    <div class="tab-pane " id="tabItem2">
        <div class="nk-msg-list" >
            <template v-for="quote in orderedUsers" v-if="quote.quote.status == 'done'" >
            <div class="nk-msg-item " v-bind:class="{current: quote.id == quoteChat.id }" v-on:click="openChat($event,quote)">
                <p style="display:none">{{doneJob + 1}}</p>
               <div class="nk-msg-media user-avatar" v-if="quote.chatsp.user_img">
                      <img :src="hostname+'/frontend-assets/images/users/'+quote.chatsp.user_img" alt="">
                    </div>
                    <div class="user-avatar bg-purple" v-else>
...
<div class="nk-chat-panel" >
 ...
 <div class="nk-msg-head d-none d-lg-block">
    <div class="nk-msg-head-meta" v-if="quoteChat.chatsp">
       <div class=" d-none d-lg-block">
                <a href="#" class="d-flex align-items-center">
                    ...
                    <div class="user-avatar sq bg-purple" v-else><span>{{getFirstLetter(quoteChat.chatsp.first_name)}}{{getFirstLetter(quoteChat.chatsp.last_name)}}</span></div>
                    <div class="ml-3">
  ...
  <div class="nk-chat-body" id="chatbody" style="height: 530px;">
    <div class="nk-chat-panel" id="chatpanelbody" v-if="loginUser" >
       <template v-for="chatData in quoteChat.chat">
           <div class="card bg-light mb-4" v-if="chatData.messageStart == 1">
            <div class="card-header">Quote Receive</div>
...
sockets: {
       ...
       receiveMsg(data){
               ...
               if(data.sender_id == this.quoteChat.user_id && data.quotation_id == this.quoteChat.id){
                   }else{
...
methods: {
           ...
           if (this.editChatid) {
                  ...
                  this.editChatid = '';
                   ...
                   var obj = {
                       ...
                       'receiver_id': this.quoteChat.user_id,
                       'message': this.message,
                       'messageType': '0',
                       'messageStart': '0',
                       'isDeleted': '0',
        ...
        uploadfile(event) {
            ...
            formDatas.append('receiver_id', this.quoteChat.user_id);
        ...
        openChat(e,quote){
            ...
            var msgObj = {
                    ...
                    user_id: this.quoteChat.user_id,
        ...
        deleteMsg(id){
               ...
               axios.get('/deleteMsg/'+id)
                .then((response) => {
                this.$socket.emit('deleteMsg', response.data);
        ...
        changeStatus(status){
            ...
            const post = this.activeQuotes.filter((obj) => {
                return this.quoteChat.id === obj.id;
                }).pop(post);
               console.log(post);
            ...
            var obj = {
                'quotation_id': this.quoteChat.id,
                'quote_id': this.quoteChat.quote_id,
                'provider_id': this.quoteChat.user_id,
                'status': status,
                '_token' : $('meta[name="csrf-token"]').attr('content'),
            }

            axios.post('/customer/change-status', obj).then((response) => {
            console.log(response.data);
            this.$toast.open({
