@if (!$server->id)
<form id="editServerForm" action="{{ url('sending_servers/create', ["type" => request()->type]) }}" method="POST" class="form-validate-jqueryz">
    {{ csrf_field() }}
    <input type="hidden" name="type" value="{{ $server->type }}" />
@else
<form id="editServerForm" enctype="multipart/form-data" action="{{ url('sending_servers', [$server->uid, $server->type]) }}" method="POST" class="form-validate-jqueryz">
...
@endif
    ...
    <div class="mc_section">
        ...
        <div class="text-left">
            ...
            @else
                <button class="btn btn-secondary me-2">{{ trans('messages.save') }}</button>
                <a href="{{ url('sending_servers') }}" role="button" class="btn btn-link">
                    {{ trans('messages.cancel') }}
                </a>
            @endif
