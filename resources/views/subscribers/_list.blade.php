			...
			<tr>
				...
				<td>
					<div class="d-flex">
						<div class="subscriber-avatar">
							<a href="{{ url('lists/'.$list->uid.'/subscribers/'.$item->uid.'/edit') }}">
						...
						<div class="no-margin text-bold">
							<a class="kq_search" href="{{ url('lists/'.$list->uid.'/subscribers/'.$item->uid.'/edit') }}">
								{{ $item->email }}
							</a>
							<br />
				...
				<td class="text-end text-nowrap">
					@if (\Gate::allows('update', $item))
						<a href="{{ url('lists/'.$list->uid.'/subscribers/'.$item->uid.'/edit') }}" role="button" class="btn btn-secondary btn-icon">
					...
					<div class="btn-group">
						...
						<ul class="dropdown-menu dropdown-menu-end">
							@if (\Gate::allows('subscribe', $item))
								<li><a class="dropdown-item list-action-single"  link-method="POST" href="{{ url('lists/'.$list->uid.'/subscribers/subscribe', ['list_uid' => $list->uid, "uids" => $item->uid]) }}"><span class="material-icons-outlined">
mark_email_read
</span> {{ trans('messages.subscribe') }}</a></li>
							@endif
							@if (\Gate::allows('unsubscribe', $item))
								<li><a class="dropdown-item list-action-single"  link-method="POST" href="{{ url('lists/'.$list->uid.'/subscribers/unsubscribe', ['list_uid' => $list->uid, "uids" => $item->uid]) }}"><span class="material-icons-round">
logout
							...
							<li>
								<a href="#copy" class="dropdown-item copy_move_subscriber"
									data-url="{{ url('lists/'.$item->uid.'/copy-move-from/copy', [
										'uids' => $item->uid,
										'from_uid' => $list->uid,
										'action' => 'copy',
									]) }}">
...
copy_all
							...
							<li>
								<a href="#move" class="dropdown-item copy_move_subscriber"
									data-url="{{ url('lists/'.$item->uid.'/copy-move-from/move', [
										'uids' => $item->uid,
										'from_uid' => $list->uid,
										'action' => 'move',
									]) }}">
...
exit_to_app
							...
							@if (\Gate::allows('update', $item))
								<li>
									<a class="dropdown-item list-action-single" link-method="POST" link-confirm="{{ trans('messages.subscribers.resend_confirmation_email.confirm') }}" href="{{ url('lists/'.$list->uid.'/subscribers/resend/confirmation-email/'.$item->uid, ['list_uid' => $list->uid, "uids" => $item->uid]) }}">
										<span class="material-icons-outlined">
mark_email_read
							...
							@if (\Gate::allows('delete', $item))
								<li><a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.delete_subscribers_confirm') }}" href="{{ url('lists/'.$list->uid.'/subscribers/delete', ['list_uid' => $list->uid, "uids" => $item->uid]) }}"><span class="material-icons-outlined">
delete_outline
