			...
			<tr>
				...
				<td>
					<div class="d-flex">
						<div class="subscriber-avatar">
							<a href="{{ action('Admin\SubscriberController@edit', ['list_uid' => $list->uid ,'uid' => $item->uid]) }}">
						...
						<div class="no-margin text-bold">
							<a class="kq_search" href="{{ action('Admin\SubscriberController@edit', ['list_uid' => $list->uid ,'uid' => $item->uid]) }}">
								{{ $item->email }}
							</a>
							<br />
				...
				<td class="text-end text-nowrap">
					@if (\Gate::allows('update', $item))
						<a href="{{ action('Admin\SubscriberController@edit', ['list_uid' => $list->uid, "uid" => $item->uid]) }}" role="button" class="btn btn-secondary btn-icon">
					...
					<div class="btn-group">
						...
						<ul class="dropdown-menu dropdown-menu-end">
							@if (\Gate::allows('subscribe', $item))
								<li><a class="dropdown-item list-action-single"  link-method="POST" href="{{ action('Admin\SubscriberController@subscribe', ['list_uid' => $list->uid, "uids" => $item->uid]) }}"><span class="material-icons-outlined">
mark_email_read
</span> {{ trans('messages.subscribe') }}</a></li>
							@endif
							@if (\Gate::allows('unsubscribe', $item))
								<li><a class="dropdown-item list-action-single"  link-method="POST" href="{{ action('Admin\SubscriberController@unsubscribe', ['list_uid' => $list->uid, "uids" => $item->uid]) }}"><span class="material-icons-round">
logout
							...
							<li>
								<a href="#copy" class="dropdown-item copy_move_subscriber"
									data-url="{{ action('Admin\SubscriberController@copyMoveForm', [
										'uids' => $item->uid,
										'from_uid' => $list->uid,
										'action' => 'copy',
									]) }}">
...
exit_to_app
							...
							@if (\Gate::allows('update', $item))
								<li>
									<a class="dropdown-item list-action-single" link-method="POST" link-confirm="{{ trans('messages.subscribers.resend_confirmation_email.confirm') }}" href="{{ action('Admin\SubscriberController@resendConfirmationEmail', ['list_uid' => $list->uid, "uids" => $item->uid]) }}">
										<span class="material-icons-outlined">
mark_email_read
							...
							@if (\Gate::allows('delete', $item))
								<li><a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.delete_subscribers_confirm') }}" href="{{ action('Admin\SubscriberController@delete', ['list_uid' => $list->uid, "uids" => $item->uid]) }}"><span class="material-icons-outlined">
delete_outline
