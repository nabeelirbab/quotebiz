...
@section('content')
    @if (count($subscribers))
        ...
        <form enctype="multipart/form-data" action="{{ action('Admin\SubscriberController@delete', $list->uid) }}" method="POST"
            ...
            <ul class="subscriber-list">
                @foreach($subscribers as $subscriber)
                    <li class="d-flex align-items-center">
                        ...
                        <img class="avatar" src="{{ (isSiteDemo() ? 'https://i.pravatar.cc/300?v=' . $key : action('Admin\SubscriberController@avatar',  $subscriber->uid)) }}" />
                        <div>
    ...
    <script>
        bulkDeletePopup.back = function() {
            $.ajax({
                url: '{{ action('Admin\SubscriberController@bulkDelete', $list->uid) }}',
                method: 'GET',
                data: {
