...
@section('title')
    <span class="material-icons-outlined me-1 text-muted2">copy</span> {!! trans('messages.copy_campaign', [
        'name' => $campaign->name
    ]) !!}
@endsection
...
@section('content')
        <form id="copyCampaginForm"
            action="{{ action('Admin\CampaignController@copy', ['copy_campaign_uid' => $campaign->uid]) }}"
            method="POST">
            {{ csrf_field() }}
