...
@section('content')
    <div id="CampaignsIndexContainer" class="listing-form top-sticky"
        data-url="{{ action('Admin\CampaignController@listing') }}"
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownListActions">
                                <li>
                                    <a class="action dropdown-item"
                                        link-method="POST"
                                        link-confirm="{{ trans('messages.restart_campaigns_confirm') }}" href="{{ action('Admin\CampaignController@restart') }}">
                                        <span class="material-icons-outlined me-2">
restore
</span> {{ trans("messages.restart") }}</a></li>
                                <li><a class="action dropdown-item"
                                    link-method="POST"
                                    link-confirm="{{ trans('messages.pause_campaigns_confirm') }}" href="{{ action('Admin\CampaignController@pause') }}">
                                    <span class="material-icons-outlined me-2">
motion_photos_pause
</span> {{ trans("messages.pause") }}</a></li>
                                <li><a class="action dropdown-item"
                                    link-method="POST"
                                    link-confirm="{{ trans('messages.delete_campaigns_confirm') }}" href="{{ action('Admin\CampaignController@delete') }}">
                                    <span class="material-icons-outlined me-2">
delete_outline
...
search
            ...
            <div class="text-end">
                <a href="{{ action('Admin\CampaignController@selectType') }}" role="button" class="btn btn-secondary">
                    <span class="material-icons-round">
add
    ...
    <script>
        var CampaignsIndex = {
            getList: function() {
                return makeList({
                    url: '{{ action('Admin\CampaignController@listing') }}',
