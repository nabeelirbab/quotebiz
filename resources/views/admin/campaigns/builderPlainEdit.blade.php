...
@section('menu_right')
    ...
    <li class="nav-item">
        <a href="{{ action('Admin\CampaignController@builderClassic', [
            'uid' => $campaign->uid
        ]) }}"
            class="nav-link d-flex align-items-center py-3 lvl-1">
...
@section('content')
    <form style="" id="classic-builder-form" action="{{ action('Admin\CampaignController@builderPlainEdit', $campaign->uid) }}" method="POST" class="form-validate-jqueryz">
        {{ csrf_field() }}
            ...
            <div class="col-md-3 pr-0 pb-0 sidebar pr-4 pt-4 pl-4" style="overflow:auto;background:#f5f5f5">
                <div>
                    @if ($campaign->plain == null)
                        ...
                        <div class="mb-4">
                            <a href="{{ action('Admin\CampaignController@customPlainOn', [
                                'uid' => $campaign->uid,
                    ...
                    @else
                        ...
                        <div class="mb-4">
                            <a href="{{ action('Admin\CampaignController@customPlainOff', [
                                'uid' => $campaign->uid,
                            ]) }}" link-method="POST" class="btn btn-primary">
                                <span class="material-icons-outlined me-1">
                                    post_add
