...
@section('content')
        ...
        <div class="right">
            <form action="{{ action('Admin\CampaignController@template', $campaign->uid) }}" method="POST" class="form-validate-jqueryz">
                {{ csrf_field() }}
                ...
                <div class="">
                    ...
                    <a href="{{ action('Admin\CampaignController@templatePreview', $campaign->uid) }}" class="btn bg-slate">{{ trans('messages.cancel') }}</a>
                </div>
            </form>
        </div>
        <div class="left">
