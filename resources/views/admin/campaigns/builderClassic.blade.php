...
@section('menu_right')
    ...
    <li class="nav-item">
        <a href="{{ action('Admin\CampaignController@builderPlainEdit', [
            'uid' => $campaign->uid
        ]) }}"
            class="nav-link d-flex align-items-center py-3 lvl-1">
...
@section('content')
    {{-- <header>
		<nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark" style="height: 56px;">
			...
			<div class="automation-top-menu">
                <a class="action mr-4" href="{{ action('Admin\CampaignController@builderPlainEdit', [
                    'uid' => $campaign->uid
                ]) }}">
    ...
    <form id="classic-builder-form" action="{{ action('Admin\CampaignController@builderClassic', $campaign->uid) }}" method="POST" class="form-validate-jqueryz">
        {{ csrf_field() }}
