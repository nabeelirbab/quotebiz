...
@section('page_header')
    ...
    <div class="page-title">
        <ul class="breadcrumb breadcrumb-caret position-right">
            ...
            <li class="breadcrumb-item"><a href="{{ action("Admin\CampaignController@index") }}">{{ trans('messages.campaigns') }}</a></li>
        </ul>
        <h1>
...
@section('content')
    ...
    <div class="confirm-campaign-box">
        <form action="{{ action('Admin\CampaignController@confirm', $campaign->uid) }}" method="POST" class="form-validate-jqueryz">
            {{ csrf_field() }}
            ...
            <ul class="modern-listing">
                @if (!is_null($score))
                    ...
                    <li class="d-flex align-items-center">
                        ...
                        <a href="{{ action('Admin\CampaignController@recipients', $campaign->uid) }}" class="btn btn-secondary">{{ trans('messages.edit') }}</a>
                    </li>
                @endif
                <li class="d-flex align-items-center">
    ...
    task_alt
                ...
                <li class="d-flex align-items-center">
                    ...
                    <a href="{{ action('Admin\CampaignController@setup', $campaign->uid) }}" class="btn btn-secondary">{{ trans('messages.edit') }}</a>
                </li>
            ...
            @if ($campaign->step() >= 5)
                @if (!is_null($score) && !$score['result'])
                    @if (!Acelle\Model\Setting::isYes('spamassassin.required'))
                        ...
                        <div class="text-end">
                            <span
                                onclick="popupwindow('{{ action('Admin\CampaignController@preview', $campaign->uid) }}', '{{ $campaign->name }}', 800, 800)"
...
visibility
    ...
    <script>
        ...
        var CampaignsSendTestEmailPopup = {
            ...
            url: '{{ action('Admin\CampaignController@sendTestEmail', [
                'uid' => $campaign->uid,
            ]) }}',
        ...
        var CampaignsConfirmPreviewAs = {
            popup:  null,
            url: '{{ action('Admin\CampaignController@previewAs', [
                'uid' => $campaign->uid,
            ]) }}',
