@if ($campaigns->count() > 0)
	...
	>
		@foreach ($campaigns as $key => $campaign)
			<tr>
				...
				<td>
					<a class="kq_search fw-600 d-block list-title" href="{{ action('Admin\CampaignController@show', $campaign->uid) }}">
						{{ $campaign->name }}
					</a>
				...
				<td class="text-end text-nowrap">
					<div class="d-flex align-items-center text-nowrap justify-content-end">
						@if (\Gate::allows('update', $campaign))
							<a href="{{ action('Admin\CampaignController@edit', $campaign->uid) }}" role="button"
	...
	edit
						...
						@if (\Gate::allows('overview', $campaign))
							<a href="{{ action('Admin\CampaignController@overview', $campaign->uid) }}" data-popup="tooltip"
								...
								<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="btnGroupDrop1">
									@if (\Gate::allows('resend', $campaign))
										<li>
											...
											href="{{ action('Admin\CampaignController@resend', ["uid" => $campaign->uid]) }}">
											<span class="material-icons-outlined me-2">
	restart_alt
	</span> {{ trans("messages.campaign.resend") }}</a></li>
									@endif
									@if (\Gate::allows('send_test_email', $campaign))
										<li><a class="dropdown-item send-a-test-email-link" href="{{ action('Admin\CampaignController@sendTestEmail', [
											'uid' => $campaign->uid,
										]) }}">
											<span class="material-icons-outlined me-2">
forward_to_inbox
									...
									@if (\Gate::allows('pause', $campaign))
										<li><a class="dropdown-item list-action-single"
											...
											href="{{ action('Admin\CampaignController@pause', ["uids" => $campaign->uid]) }}">
											<span class="material-icons-outlined me-2">
	motion_photos_pause
									...
									@if (\Gate::allows('restart', $campaign))
										<li><a class="dropdown-item list-action-single"
											link-method="POST"
											link-confirm="{{ trans('messages.restart_campaigns_confirm', ['number' => '1']) }}" href="{{ action('Admin\CampaignController@restart', ["uids" => $campaign->uid]) }}">
											<span class="material-icons-outlined me-2">
	restore
									...
									@if (\Gate::allows('copy', $campaign))
										<li>
											<a class="copy-campaign-button dropdown-item"
												href="{{ action('Admin\CampaignController@copy', [
													'uid' => $campaign->uid,
									...
									@if (\Gate::allows('delete', $campaign))
										<li><a
											...
											href="{{ action('Admin\CampaignController@delete', ["uids" => $campaign->uid]) }}">
											<span class="material-icons-outlined me-2">
	delete_outline
