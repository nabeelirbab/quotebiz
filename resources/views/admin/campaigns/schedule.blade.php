...
forward_to_inbox
        ...
        @include('admin.campaigns._steps', ['current' => 4])
    </div>

@endsection

@section('content')
    <form id="CampaignScheduleForm" action="{{ action('Admin\CampaignController@schedule', $campaign->uid) }}" method="POST" class="form-validate-jqueryz">
        {{ csrf_field() }}
        
        <input type="hidden" name="send_now" value="no" />
        <div class="row">
            <div class="col-md-3 list_select_box" target-box="segments-select-box" segments-url="{{ action('Admin\SegmentController@selectBox') }}">
