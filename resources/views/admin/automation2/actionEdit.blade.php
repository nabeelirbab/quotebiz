...
<form class="action-edit" action="{{ action("Admin\Automation2Controller@actionEdit", ['uid' => $automation->uid, 'key' => $key]) }}" method="POST" class="form-validate-jqueryz">
    {{ csrf_field() }}
    ...
    <div class="trigger-action mt-2">    
        <button class="btn btn-secondary action-save-change mr-1 mt-2"
            data-url="{{ action('Admin\Automation2Controller@triggerSelect', ['uid' => $automation->uid, 'key' => $key]) }}"
        >
