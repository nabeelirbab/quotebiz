...
@section('content')
    ...
    <div>
        <ul style="float:left" class="pagination" role="navigation">
            <li class="page-item">
                <a href="{{ action('Admin\Automation2Controller@debug', ['uid' => $automation->uid, 'orderBy' => 'triggered_at', 'orderDir' => 'DESC']) }}">Sort By Trigger Date</a>
            </li>
        </ul>
                ...
                <td>
                    <span class="no-margin kq_search">
                        @if (is_null($subscriber->auto_trigger_id))
                            <a href="{{ action('Admin\Automation2Controller@triggerNow', [ 'automation' => $automation->uid, 'subscriber' => $subscriber->uid ]) }}">Trigger Now</a>
                        @else
