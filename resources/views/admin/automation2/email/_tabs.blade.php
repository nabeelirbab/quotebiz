...
<ul class="nav nav-tabs mb-4 email_tabs nav-underline">
    <li class="nav-item">
        <a class="nav-link setup" href="javascript:;" onclick="popup.load('{{ action('Admin\Automation2Controller@emailSetup', [
            'uid' => $automation->uid,
            'email_uid' => $email->uid,
        ]) }}')">
...
settings
    ...
    <li class="nav-item">
        <a class="nav-link template {{ (!isset($email->id)) ? 'disabled' : '' }}" href="javascript:;"
            @if (isset($email->id))                    
                onclick="popup.load('{{ action('Admin\Automation2Controller@emailTemplate', [
                    'uid' => $automation->uid,
                    'email_uid' => $email->uid,
                ]) }}')"
            @endif
...
mark_email_unread
    ...
    <li class="nav-item">
        <a class="nav-link confirm {{ (!isset($email->id)) ? 'disabled' : '' }}" href="javascript:;"
            @if (isset($email->id))                    
                onclick="popup.load('{{ action('Admin\Automation2Controller@emailConfirm', [
                    'uid' => $automation->uid,
                    'email_uid' => $email->uid,
                ]) }}')"
            @endif
