...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form enctype="multipart/form-data" action="{{ action('Admin\Automation2Controller@templateUpload', [
                'uid' => $automation->uid,
                'email_uid' => $email->uid,
        ...
        $('.template-upload-form').submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    popup.load('{{ action('Admin\Automation2Controller@emailTemplate', [
                        'uid' => $automation->uid,
                        'email_uid' => $email->uid,
                    ]) }}');

                    builderSelectPopup.load('{{ action('Admin\Automation2Controller@templateBuilderSelect', [
                        'uid' => $automation->uid,
                        'email_uid' => $email->uid,
                    ]) }}');
