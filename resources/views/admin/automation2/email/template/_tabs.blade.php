<ul class="nav nav-tabs email-template-tabs nav-underline" id="pills-tab" role="tablist">
    @foreach (Acelle\Model\TemplateCategory::all() as $cat)
        <li class="nav-item">
            <a class="nav-link {{ request()->category_uid == $cat->uid ? 'active' : '' }}" href="javascript:;"
                data-href="{{ action('Admin\Automation2Controller@templateLayout', [
                'uid' => $automation->uid,
                'email_uid' => $email->uid,
    ...
    <li class="nav-item">
        <a class="choose-template-tab nav-link {{ actionName() == 'templateUpload' ? 'active' : '' }}" href="javascript:;" data-href="{{ action('Admin\Automation2Controller@templateUpload', [
            'uid' => $automation->uid,
            'email_uid' => $email->uid,
