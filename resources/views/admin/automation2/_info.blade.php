...
<div class="d-flex align-items-center mb-4">
    ...
    <div>
        <a href="javascript:;" class="btn btn-info text-nowrap d-flex align-items-center" onclick="sidebar.load('{{ action('Admin\Automation2Controller@settings', $automation->uid) }}')">
...
<script>
    ...
    $('[name="automation_status"]').change(function() {
        ...
        if (value) {
            url = '{{ action('Admin\Automation2Controller@enable', ["uids" => $automation->uid]) }}';
            confirm = '{!! trans('messages.automation.enable.confirm', ['name' => $automation->name]) !!}';
        } else {
            url = '{{ action('Admin\Automation2Controller@disable', ["uids" => $automation->uid]) }}';
