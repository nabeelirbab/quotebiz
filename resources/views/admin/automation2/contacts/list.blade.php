        ...
        @foreach ($contacts as $key => $contact)
            ...
            <div class="mc-row d-flex align-items-center">
                <div class="media trigger">
                    <a href="javascript:;" onclick="popup.load('{{ action('Admin\Admin\Automation2Controller@profile', [
                        'uid' => $automation->uid,
                ...
                <div class="flex-fill" style="width: 20%">
                    <a href="javascript:;" onclick="popup.load('{{ action('Admin\Automation2Controller@profile', [
                        'uid' => $automation->uid,
                        'contact_uid' => $contact->uid,
                    ]) }}')" class="font-weight-semibold d-block">
                        {{ $contact->getFullName() }}
                    ...
                    @if (is_null($trigger))
                        ...
                        <desc>
                            <a target="_blank" style="color:blue;text-decoration: underline;"
                                href="{{ action('Admin\Automation2Controller@triggerNow', [ 'automation' => $automation->uid, 'subscriber' => $contact->uid ]) }}"
