...
@section('menu_right')
    <li class="d-flex align-items-center">
        <div class="d-flex align-items-center me-4 automation-top-actions">
            <span class="me-4"><i class="last_save_time" data-url="{{ action('Admin\Automation2Controller@lastSaved', $automation->uid) }}">{{ trans('messages.automation.designer.last_saved', ['time' => $automation->updated_at->diffForHumans()]) }}</i></span>
            <a href="{{ action('Admin\Automation2Controller@index') }}" class="action me-4">
            ...
            <div class="switch-automation d-flex">
                <select class="select select2 top-menu-select" name="switch_automation">
                    ...
                    @foreach($automation->getSwitchAutomations(Auth::user()->customer)->get() as $auto)
                        <option value='{{ action('Admin\Automation2Controller@edit', $auto->uid) }}'>{{ $auto->name }}</option>
                    @endforeach
                </select>
...
@section('content')
    ...
    <script>
        ...
        function saveData(callback, extra = {}) {
            ...
            var url = '{{ action('Admin\Automation2Controller@saveData', $automation->uid) }}';
        ...
        function SelectActionConfirm(key, insertToTree) {
            ...
            var url = '{{ action('Admin\Automation2Controller@actionSelectConfirm', $automation->uid) }}' + '?key=' + key;
    ...
    message: response.message
        ...
        function EmailSetup(id) {
            var url = '{{ action('Admin\Automation2Controller@emailSetup', $automation->uid) }}' + '?action_id=' + id;
        ...
        $(document).ready(function() {
            // load sidebar
            sidebar.load('{{ action('Admin\Automation2Controller@settings', $automation->uid) }}');
        ...
        function OpenActionSelectPopup(insertToTree, conditionBranch = null) {
            ...
            popup.load('{{ action('Admin\Automation2Controller@actionSelectPupop', $automation->uid) }}?hasChildren=' + hasChildren, function() {
        ...
        function OpenTriggerSelectPopup() {
            popup.load('{{ action('Admin\Automation2Controller@triggerSelectPupop', $automation->uid) }}', function() {
        ...
        function SelectTriggerConfirm(key) {
            var url = '{{ action('Admin\Automation2Controller@triggerSelectConfirm', $automation->uid) }}' + '?key=' + key;
        ...
        function doSelect(e) {
            ...
            if (e.getType() == 'ElementTrigger') {
                ...
                // trigger was init
                    ...
                    var url = '{{ action('Admin\Automation2Controller@triggerEdit', $automation->uid) }}' + '?key=' + e.getOptions().key + '&id=' + e.getId();
            ...
            // is WAIT
                    ...
                    var url = '{{ action('Admin\Automation2Controller@actionEdit', $automation->uid) }}' + '?key=' + e.getOptions().key + '&id=' + e.getId();
            ...
            // is Email
                ...
                if (e.getOptions().init == "true") {
                    ...
                    var url = '{{ action('Admin\Automation2Controller@email', $automation->uid) }}?email_uid=' + e.getOptions().email_uid;
            ...
            // is Email
                ...
                var type = $(this).attr('data-type');
                var url = '{{ action('Admin\Automation2Controller@operationShow', $automation->uid) }}?operation=' + e.getOptions().operation_type + '&id=' + e.getId();
