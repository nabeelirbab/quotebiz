...
@include('admin.automation2._tabs', ['tab' => 'settings'])
    
<p class="mt-3">
    {!! trans('messages.automation.settings.intro') !!}
</p>
    
<form id="automationUpdate" action="{{ action("Admin\Automation2Controller@update", $automation->uid) }}" method="POST" class="form-validate-jqueryz">
    {{ csrf_field() }}
...
<div class="mt-4 d-flex py-3">
    <div>
        ...
        <div class="mt-3">
            <a href="{{ action('Admin\Automation2Controller@delete', ['uids' => $automation->uid]) }}"
...
<script>
    ...
    $('[name=mail_list_uid]').change(function(e) {
        var url = '{{ action('Admin\Automation2Controller@segmentSelect') }}?uid={{ $automation->uid }}&list_uid=' + $(this).val();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        addMaskLoading(
                            ...
                            function() {
                                window.location = '{{ action('Admin\Automation2Controller@index') }}';
                            },
                            { wait: 2000 }
                        );
