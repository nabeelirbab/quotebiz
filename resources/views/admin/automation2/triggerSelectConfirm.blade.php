...
@section('content')
	<div class="row">
        <div class="col-md-12">
			...
			<form id="trigger-select" action="{{ action("Admin\Automation2Controller@triggerSelect", $automation->uid) }}" method="POST" class="form-validate-jqueryz">
				{{ csrf_field() }}
				...
				<button class="btn btn-secondary select-trigger-confirm mt-2"
					data-url="{{ action('Admin\Automation2Controller@triggerSelect', ['uid' => $automation->uid]) }}"
				>
    ...
    message: response.message
						...
						// Edit Trigger
						EditTrigger('{{ action('Admin\Automation2Controller@triggerEdit', $automation->uid) }}' + '?key=' + tree.getOptions().key);
					}, flag);
				}
			});
		}
