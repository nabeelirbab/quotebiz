...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            <form id="trigger-select" action="{{ action("Admin\Automation2Controller@wizardTriggerOption") }}"
                method="POST"
            >
                {{ csrf_field() }}
	...
	<script>
		$(function() {
            ...
            $('#trigger-select [name=mail_list_uid]').change(function(e) {
                var url = '{{ action('Admin\Automation2Controller@segmentSelect') }}?list_uid=' + $(this).val();
