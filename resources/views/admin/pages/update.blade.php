...
@section('content')
                ...
                @if ($layout->alias == 'sign_up_form')
                    <p class="alert alert-info mt-20 mb-20">{{ trans('messages.sign_up_form_url') }}<br /> <a target="_blank" href="{{ action('Admin\PageController@signUpForm', ['list_uid' => $list->uid]) }}" class="text-semibold">{{ action('Admin\PageController@signUpForm', ['list_uid' => $list->uid]) }}</a></p>
                @endif

                <form id="update-page" action="{{ action('Admin\PageController@update', ['list_uid' => $list->uid, 'alias' => $layout->alias]) }}" method="POST" class="form-validate-jqueryz">
					{{ csrf_field() }}
						...
						<div class="form-group control-radio">
							...
							<div class="radio_box" data-popup='tooltip' title="">
								...
								<div class="radio_more_box">
									...
									<hr />
									<div class="">
										<a page-url="{{ action('Admin\PageController@preview', ['list_uid' => $list->uid, 'alias' => $layout->alias]) }}" class="btn btn-info me-1 preview-page-button" data-toggle="modal" data-target="#preview_page"><span class="material-icons-outlined">
visibility
					...
					@else
						...
						<hr />
						<div class="d-flex">
							<a page-url="{{ action('Admin\PageController@preview', ['list_uid' => $list->uid, 'alias' => $layout->alias]) }}"
...
visibility
							...
							<a href="{{ action('Admin\PageController@restoreDefault', [
								'list_uid' => $list->uid,
								'alias' => $layout->alias
							]) }}"
								link-method="POST"
