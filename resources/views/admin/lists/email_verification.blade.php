        ...
        <div class="col-md-6">
            ...
            <p>
                <a link-confirm="{{ trans('messages.reset_list_verification_confirm') }}" link-method="POST" class="btn btn-secondary"
                    href="{{ action("Admin\MailListController@resetVerification", $list->uid) }}">
        ...
        <div class="col-md-6">
            ...
            <script>
                ...
                var EmailVerificationChart = {
                    url: '{{ action('Admin\MailListController@emailVerificationChart', [
                        'uid' => $list->uid,
                    ]) }}',
                    getChart: function() {
    ...
    <script>
        var AcelleList = {
            ...
            start: function() {
                ...
                $.ajax({
                    url: '{{ action('Admin\MailListController@startVerification', [ 'uid' => $list->uid ] ) }}',
                    type: 'POST',
                    data: {
                        '_token': token,
            ...
            cancel: function() {
                ...
                $.ajax({
                    ...
                    success: function(result, textStatus, jqXHR) {
                        ...
                        window.location.href = '{{ action("Admin\MailListController@verification", [ "uid" => $list->uid ]) }}';
                    }
                }).fail(function(jqXHR, textStatus, errorThrown) {
                    alert(errorThrown);
