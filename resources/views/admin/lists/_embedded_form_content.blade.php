...
<div class="subscribe-embedded-form">
	...
	<p class="text-sm text-end"><span class="text-danger">*</span> {{ trans('messages.indicates_required') }}</p>
		@if (!isset($preview))
			<form action="{{ action('Admin\MailListController@embeddedFormSubscribe', $list->uid) }}" method="POST" class="form-validate-jqueryz">
		@endif
						...
						@if ($list->getEmbeddedFormOption('javascript') == 'yes')			
							$(".subscribe-embedded-form form").validate({
								...
								EMAIL: {
									required: true,
									email: true,
									remote: "{{ action('Admin\MailListController@checkEmail', $list->uid) }}"
								}
								}
							});
						@endif
