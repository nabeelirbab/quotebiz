            ...
            <input type="hidden" name="category_id" value="{{$question->category_id}}"><div class="removeQuestion mt-4" style="background: #f5f6fa;padding: 20px;border-radius:15px">
             <div class="row mb-4">
                ...
                <div class="col-xs-6 col-md-4" style="padding-right: 0;"><span title="Remove Question" class="deleteQuestions material-icons-round remove-icon xtooltip tooltipstered lh-1 float-end fs-1" style="cursor: pointer;">remove_circle</span><input type="hidden" name="question_id[{{$key}}]" value="{{$question->id}}"></div> </div>
                <div class="row">
                    ...
                    <div class="appendChoiceInput col-sm-2 pr-1"><span title="Add Option" class="addChoice material-icons-round add-icon xtooltip tooltipstered  float-end fs-3" style="cursor: pointer;line-height:3!important">add_circle</span><input type="hidden" value="{{$key}}"></div>
                 ...
                 @foreach($question->choices as $key2=>$choice)
                    ...
                    <div class="col-sm-2 pr-1"><span title="Remove Option" class="deleteChoice material-icons-round remove-icon xtooltip tooltipstered  float-end fs-3" style="cursor: pointer;line-height:3!important">remove_circle</span><input type="hidden" value="{{$choice->id}}" name="choice_id[{{$key}}][]"></div>
                    </div>
                    @endforeach
                    @endif
                </div>
