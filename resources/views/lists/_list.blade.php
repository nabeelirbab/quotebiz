	...
	>
		@foreach ($lists as $key => $list)
			<tr>
				...
				<td width="35%">
					<a class="kq_search fw-600 d-block list-title" href="{{ url('lists/'.$list->uid.'/overview') }}">
						{{ $list->name }}
					</a>
...
error_outline
				...
				<td class="stat-fix-size-sm">
					<div class="d-flex">
						<div class="single-stat-box pull-left">
							<a href="{{ url('lists/'.$list->uid.'/subscribers') }}" class="d-block">
				...
				<td class="text-end pe-0">
					<div class="d-flex align-items-center text-nowrap justify-content-end" role="group">
						<a href="{{ url('lists/'.$list->uid.'/subscribers/create') }}" data-popup="tooltip"
							title="{{ trans('messages.create_subscriber') }}" role="button" class="btn btn-secondary btn-icon me-1">
							<span class="material-icons-outlined">
								person_add
							</span>
						</a>
						<a href="{{ url('lists/'.$list->uid.'/overview') }}" role="button" class="btn btn-primary me-1">
							<span class="material-icons-outlined">
								multiline_chart
						...
						<div class="btn-group" role="group">
							...
							<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="btnGroupDrop1">
								...
								<li>
									<a class="dropdown-item" href="{{ url('lists/'.$list->uid.'/fields') }}">
										<span class="material-icons-outlined me-2">
											view_day
										</span> {{ trans('messages.manage_list_fields') }}
									</a>
								</li>
								<li><a class="dropdown-item" href="{{ url('lists/$list->uid/verification') }}">
									<span class="material-icons-outlined me-2">
										gpp_good
									</span> {{ trans("messages.email_verification") }}</a></li>
								<li><a class="dropdown-item" href="{{ url('lists/$list->uid/edit') }}">
									<span class="material-icons-outlined me-2">
										edit
									</span> {{ trans("messages.edit_list") }}</a></li>
								@if (\Auth::user()->can('import', $list))
									<li><a class="dropdown-item" href="{{ url('lists/'.$list->uid.'/subscribers/import') }}">
										<span class="material-icons-outlined me-2">
											drive_folder_upload
										</span> {{ trans('messages.import') }}</a></li>
								@endif
								@if (\Auth::user()->can('export', $list))
									<li><a class="dropdown-item" href="{{ url('lists/'.$list->uid.'/subscribers/export') }}">
										<span class="material-icons-outlined me-2">
											downloading
								...
								<li>
									<a class="copy-list-button dropdown-item"
										href="{{ url('lists/copy', ['copy_list_uid' => $list->uid]) }}"
									>
										<span class="material-icons-outlined me-2">
											copy_all
								...
								<li>
									<a class="dropdown-item list-action-single"
										link-method="POST"
										link-confirm-url="{{ url('lists/delete/confirm', ['uids' => $list->uid]) }}"
										href="{{ url('lists/delete', ['uids' => $list->uid]) }}">
										<span class="material-icons-outlined me-2">
											delete
