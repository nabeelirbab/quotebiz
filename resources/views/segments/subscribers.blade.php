...
people
	...
	<div class="listing-form" id="SegmentsSubscribersContainer"
		data-url="{{ url('lists/'.$list->uid.'/segments/'.$segment->uid.'/listing_subscribers') }}"
					...
					<div class="filter-box">
						<div class="btn-group list_actions me-2" style="display:none">
							...
							<ul class="dropdown-menu dropdown-menu-end">
								<li>
									<a  link-method="POST" link-confirm="{{ trans('messages.subscribe_subscribers_confirm') }}" href="{{ url('lists/'.$list->uid.'/subscribers/subscribe') }}">
...
mark_email_read
								...
								<li>
									<a link-confirm="{{ trans('messages.unsubscribe_subscribers_confirm') }}" href="{{ url('lists/'.$list->uid.'/subscribers/unsubscribe') }}">
...
logout
								...
								<li>
									<a link-confirm="{{ trans('messages.delete_subscribers_confirm') }}" href="{{ url('lists/'.$list->uid.'/subscribers/delete') }}">
										<span class="material-icons-outlined">
delete_outline
...
add
	...
	<script>
		var SegmentsSubscribers = {
			getList: function() {
				return makeList({
					url: '{{ url('lists/'.$list->uid.'/segments/'.$segment->uid.'/listing_subscribers') }}',
