...
add
	...
	<form action="{{ url('lists/'.$list->uid.'/segments/store') }}" method="POST" class="form-validate-jqueryz">
		{{ csrf_field() }}
		...
		<div class="row">
			<div class="col-md-12">
				...
				<div class="text-left">
					...
					<a href="{{ url('lists/'.$list->uid.'/segments') }}" class="btn btn-link"><i class="icon-cross2"></i> {{ trans('messages.cancel') }}</a>
				</div>
			</div>
		</div>
	<form>
