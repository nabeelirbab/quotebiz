...
@section('content')
                ...
                @if ($layout->alias == 'sign_up_form')
                    <p class="alert alert-info mt-20 mb-20">{{ trans('messages.sign_up_form_url') }}<br /> <a target="_blank" href="{{ url('lists/'.$list->uid.'/sign-up', ['list_uid' => $list->uid]) }}" class="text-semibold">{{ url('lists/'.$list->uid.'/sign-up', ['list_uid' => $list->uid]) }}</a></p>
                @endif

                <form id="update-page" action="{{ url('lists/'.$list->uid.'/pages/'.$layout->alias.'/update', ['list_uid' => $list->uid, 'alias' => $layout->alias]) }}" method="POST" class="form-validate-jqueryz">
					{{ csrf_field() }}
						...
						<div class="form-group control-radio">
							...
							<div class="radio_box" data-popup='tooltip' title="">
								...
								<div class="radio_more_box">
									...
									<hr />
									<div class="">
										<a page-url="{{ url('lists/{list_uid}/pages/{alias}/preview', ['list_uid' => $list->uid, 'alias' => $layout->alias]) }}" class="btn btn-info me-1 preview-page-button" data-toggle="modal" data-target="#preview_page"><span class="material-icons-outlined">
visibility
					...
					@else
						...
						<hr />
						<div class="d-flex">
							<a page-url="{{ url('lists/{list_uid}/pages/{alias}/preview', ['list_uid' => $list->uid, 'alias' => $layout->alias]) }}"
...
visibility
							...
							<a href="{{ url('lists/{list_uid}/pages/{alias}/restore-default', [
								'list_uid' => $list->uid,
								'alias' => $layout->alias
							]) }}"
								link-method="POST"
