    ...
    public function message()
    {
        ...
        if ($this->type == 'campaign') {
            if (isset($data['list_id'])) {
                ...
                $list_link = is_object($list) ? url('lists/'.$list->uid.'/overview') : '#deleted';
        ...
        elseif ($this->type == 'subscriber') {
            ...
            $link = is_object($item) && is_object($list) ? url('lists/'.$item->uid.'/subscribers/'.$list->uid.'/edit') : '#deleted';
            $data['link'] = $link;
        }
        // Segment
        elseif ($this->type == 'segment') {
            ...
            $link = is_object($item) && is_object($list) ? url('lists/'.$item->uid.'/segments/'.$list->uid.'/edit') : '#deleted';
            $data['link'] = $link;
        }
        // Page
        elseif ($this->type == 'page') {
            ...
            $link = is_object($item) && is_object($list) ? url('lists/'.$list->uid.'/pages/'.$data['alias'].'/update') : '#deleted';
            $data['link'] = $link;
        ...
        elseif ($this->type == 'list') {
            ...
            $link = is_object($item) ? url('lists/'.$item->uid.'/overview') : '#deleted';
            $data['link'] = $link;

            if (isset($data['from_uid'])) {
                $from = MailList::findByUid($data['from_uid']);
                $from_link = is_object($from) ? url('lists/'.$data['from_uid'].'/overview') : '#deleted';
                $data['from_link'] = $from_link;
            }
            if (isset($data['to_uid'])) {
                $to = MailList::findByUid($data['to_uid']);
                $to_link = is_object($to) ? url('lists/'.$data['to_uid'].'/overview') : '#deleted';
