...
east
        ...
        if(\Auth::user()->admin){
            $this->content = str_replace('{SUBSCRIBE_URL}', config('app.url').action('Admin\PageController@signUpForm', $this->mailList->uid, false), $this->content);

        }else{
            $this->content = str_replace('{SUBSCRIBE_URL}', config('app.url').url('lists/'.$this->mailList->uid.'/sign-up'), $this->content);
        }
